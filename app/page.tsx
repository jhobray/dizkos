...
function analyzeFromFrames(readiness: number, frames: FrameLandmarks[], durationSeconds: number) {
  ...
  const fullBodyVisible = frames.filter((frame) => {
    const pts = [frame.leftShoulder, frame.rightShoulder, frame.leftHip, frame.rightHip, frame.leftKnee, frame.rightKnee, frame.leftAnkle, frame.rightAnkle];
    return pts.every((pt) => pt.visibility > 0.45 && pt.y >= 0 && pt.y <= 1);
  }).length / Math.max(frames.length, 1) >= 0.75;
  const avgConfidence = mean(frames.flatMap((frame) =>
    [frame.leftShoulder, frame.rightShoulder, frame.leftHip, frame.rightHip, frame.leftKnee, frame.rightKnee, frame.leftAnkle, frame.rightAnkle].map((pt) => pt.visibility)
  ));
  const movementDetected = (Math.max(...hipCenters.map((pt) => pt.x)) - Math.min(...hipCenters.map((pt) => pt.x))) > 0.04;
  ...
  const hipDrop = bandThree(mean(frames.map((frame) => Math.abs(frame.leftHip.y - frame.rightHip.y) * 100)), 3.5, 6.5);
  ...
  const asymmetryValue = Math.abs(mean(leftStrideSeries) - mean(rightStrideSeries)) / Math.max((mean(leftStrideSeries) + mean(rightStrideSeries)) / 2, 0.0001) * 100;
  const asymmetry = bandThree(asymmetryValue, 6, 11);
  const trunkOffsets = frames.map((frame) => Math.abs(averagePoint(frame.leftShoulder, frame.rightShoulder).x - averagePoint(frame.leftHip, frame.rightHip).x));
  const trunkPosition = mean(trunkOffsets) > 0.045 ? "Inestabilidad" : mean(trunkOffsets) > 0.025 ? "Leve inclinación" : "Estable";
  const impactControl = bandThree(mean(frames.map((frame) => {
  ...
  const cadence = clamp(Math.round((Math.max(8, Math.round(durationSeconds * 2.8)) / Math.max(durationSeconds, 1)) * 60), 150, 186);
  const readinessAdjustment = readiness >= 70 ? "Lista para progresar ligeramente" : readiness >= 55 ? "Mantener carga moderada" : "Conviene reducir intensidad esta semana";
  const scorePenalty =
  ...
  return {
    ...
    diagnostics: {
      ...
      fullBodyVisible,
      notes: [
        validLateralView ? "La vista lateral es válida para analizar la técnica." : "La vista no parece completamente lateral; conviene regrabar más de perfil.",
        fullBodyVisible ? "El cuerpo completo se mantiene visible." : "El cuerpo no se ve completo durante todo el video.",
        avgConfidence >= 0.6 ? `La confianza de landmarks es suficiente (${Math.round(avgConfidence * 100)}%).` : "La confianza de landmarks es baja; revisa luz o calidad de video.",
...
async function runRealPoseAnalysis(file: File, readiness: number) {
  ...
  try {
    ...
    if (!frames.length) throw new Error("No se detectaron landmarks suficientes para analizar el video.");
  ...
  return (
    <>
      {points.map((dot, index) => (
        <div key={index} className="absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-400 shadow-[0_0_0_4px_rgba(16,185,129,0.18)]" style={{ left: `${dot.x * 100}%`, top: `${dot.y * 100}%` }} />
      ))}
...
export default function HomePage() {
  ...
  const [historyByAthlete, setHistoryByAthlete] = useState<Record<string, AnalysisRecord[]>>(() => readLocalHistory());
  ...
  const handleRunPipeline = async () => {
    ...
    try {
      ...
      if (realAnalysis.overlayImage) {
        ...
        const overlayFile = new File([overlayBlob], `overlay-${Date.now()}.jpg`, { type: overlayBlob.type || "image/jpeg" });
      ...
      setHistoryByAthlete((prev) => ({ ...prev, [athleteId]: [record, ...(prev[athleteId] || [])].slice(0, 12) }));
      setPersistenceMode("backend");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-violet-50 text-slate-900">
      <div className="mx-auto grid min-h-screen max-w-7xl gap-6 p-6 lg:grid-cols-[260px_1fr]">
        <aside className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
          ...
          <p className="mt-2 text-sm text-slate-600">Análisis real con MediaPipe, overlay temporal, historial persistente y Garmin conectado por backend opcional.</p>
          ...
          <label className="mt-5 block rounded-2xl bg-slate-50 p-4">
            ...
            <select value={athleteId} onChange={(e) => setAthleteId(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
              {athletes.map((item) => (
          ...
          <div className="mt-5 space-y-2">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-medium transition ${activeTab === tab.id ? "bg-violet-100 text-violet-700" : "bg-slate-50 text-slate-700 hover:bg-slate-100"}`}>
          ...
          {activeTab === "video" && (
            <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
              <Panel title="Subida y persistencia" icon={Upload}>
                ...
                <button onClick={handleRunPipeline} disabled={!selectedFile || isSaving} className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-5 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">
                ...
                <div className="overflow-hidden rounded-3xl bg-slate-950">
                  <div className="relative aspect-video w-full bg-black">
                    {playerSource ? (
                      ...
                      <div className="flex h-full items-center justify-center text-sm text-slate-400">Sube un video para previsualizarlo</div>
                    )}
                    {currentOverlayPoints.length ? <OverlaySkeleton points={currentOverlayPoints} /> : null}
                  </div>
                </div>
                <div className="mt-4 flex flex-wrap gap-3">
                  <button onClick={handlePlayPause} className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700">
                    {isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
                    {isPlaying ? "Pausar" : "Reproducir"}
                  </button>
                  <button onClick={handleReset} className="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700">
            ...
            <section className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
              <Panel title="Análisis actual" icon={Gauge}>
                {currentRecord ? (
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Todavía no hay un análisis activo.</div>
                )}
              </Panel>
            ...
            <section className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
              ...
              <Panel title="Notas de Dizkos" icon={TrendingUp}>
                <div className="space-y-3">
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">{currentRecord?.analysis.readinessAdjustment || "Todavía no hay ajuste de readiness calculado."}</div>
                </div>
              </Panel>
            </section>
          )}
          ...
          {activeTab === "garmin" && (
            <section className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
              <Panel title="Conexión Garmin" icon={Watch}>
                ...
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Último entreno: {garminData?.recentRun || "Sin datos"}</div>
                <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">{garminData?.lastSyncLabel || "Sin información de sincronización"}</div>
              </Panel>
              <Panel title="Cómo impacta al plan" icon={Flag}>
                <div className="space-y-3">
                  {[
                    (garminData?.readiness ?? 0) >= 70 ? "La carga semanal puede progresar ligeramente." : (garminData?.readiness ?? 0) >= 55 ? "Conviene sostener una carga moderada." : "La prioridad es recuperar y bajar intensidad.",
                    garminData?.connected ? "La app ya tiene un contexto premium del atleta para personalizar el bloque semanal." : "Sin Garmin, Dizkos usa reglas conservadoras y feedback manual.",
                    currentRecord ? `El análisis actual se cruza con una cadencia estimada de ${currentRecord.cadence ?? "—"} spm.` : "Todavía no hay análisis para cruzar con los datos del reloj.",
                  ].map((item) => (
                    <div key={item} className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">{item}</div>
                  ))}
                </div>
            ...
            <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">Modo actual: {persistenceMode === "backend" ? "Backend real" : "Caché local"}</p>
                </div>
                <button onClick={clearHistory} className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700">
                  Limpiar historial
                </button>
              </div>
                ...
                {history.length ? (
                  history.map((item, index) => (
                    <button
                      ...
                      className={`block w-full rounded-2xl border p-4 text-left ${selectedHistoryIndex === index ? "border-violet-400 bg-violet-50" : "border-slate-200 bg-white"}`}
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-slate-900">Análisis {index + 1}</p>
                          <p className="mt-1 text-sm text-slate-600">Score: {item.score}/100 · Cadencia: {item.cadence ?? "—"} spm</p>
                          <p className="mt-1 text-xs text-slate-500">Motor: {item.engine} · {item.analyzedAt ? new Date(item.analyzedAt).toLocaleString() : "sin fecha"}</p>
                        </div>
                        <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">
                          {selectedHistoryIndex === index ? <CheckCircle2 className="h-3.5 w-3.5" /> : <Sparkles className="h-3.5 w-3.5" />}
                  ...
                  <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Todavía no hay análisis persistidos para este atleta.</div>
                )}
              </div>
            </section>
          )}
